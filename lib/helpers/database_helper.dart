...
class DatabaseHelper {
  ...
  Future<Database> _initDB(String filePath) async {
    final path = _testDatabasePath ?? join(await getDatabasesPath(), filePath);

    return await openDatabase(path, version: 1, onCreate: _createDB);
  }

  /// Mengarahkan DB ke path unik agar unit test tidak bentrok antar file test.
  @visibleForTesting
  static Future<void> useIsolatedTestDatabase(String uniqueName) async {
    await closeForTesting();
    final dbPath = await getDatabasesPath();
    final path = join(dbPath, 'test_$uniqueName.db');
  ...
  @visibleForTesting
  static void clearTestDatabasePath() {
  ...
  /// Menutup koneksi DB agar unit test dapat memakai instance bersih.
  @visibleForTesting
